...
class DevServerLeaderboard : Interaction() {

    override val id =
        "dev-server-leaderboard"
    override val developer = true
    override val commandData =
        Commands.slash("dev-server-leaderboard", "Check the leaderboard of a server")
            .addOption(
                OptionType.STRING,
                "server-name",
    ...
    override fun execute(context: InteractionContext) {
        ...
        when {
            ...
            key != null && overrideServer == null -> {
                ...
                val group = (overrideServer ?: context.server).group
